...
export function imagePoint(
  ...
  return [
    (client.x - bounds.left - view.x) / view.scale - image.x,
    (client.y - bounds.top - view.y) / view.scale - image.y,
  ];
}
...
export function connectionPath(parent: NodeBounds, child: NodeBounds): string {
  ...
  const bend = Math.max(80, Math.abs(end[0] - start[0]) * 0.45) * (right ? 1 : -1);
