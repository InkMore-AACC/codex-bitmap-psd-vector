  ...
  if (Array.isArray(base) && Array.isArray(current) && Array.isArray(saved)) {
    if (current.every(x => x && typeof x === 'object' && typeof x.id === 'string')) {
      const merged = current.map(item => {
        const before = base.find(x => x.id === item.id); const after = saved.find(x => x.id === item.id);
        return before && after ? reconcileSaved(before, item, after) : item;
      });
      for (const item of saved) if (!base.some(x => x.id === item.id) && !current.some(x => x.id === item.id)) merged.push(item);
      return merged as T;
    }
    return current;
  }
  if (base && current && saved && typeof base === 'object' && typeof current === 'object' && typeof saved === 'object' && !Array.isArray(current)) {
    const out: Record<string, unknown> = {};
    for (const key of new Set([...Object.keys(current), ...Object.keys(saved)])) {
      const b = (base as any)[key], c = (current as any)[key], s = (saved as any)[key];
...
export function mergeConcurrent<T>(base: T, local: T, remote: T): { value: T; conflicts: string[] } {
  ...
  const merge = (b: any, l: any, r: any, at: string): any => {
    ...
    if (Array.isArray(b) && Array.isArray(l) && Array.isArray(r) && [...b, ...l, ...r].every(x => x && typeof x.id === 'string')) {
      ...
      if (!equal(localOrder, remainingBase) && !equal(remoteOrder, remainingBase) && !equal(localOrder, remoteOrder)) conflicts.push(`${at}.order`);
      ...
      return order.map(id => merge(b.find(x => x.id === id), l.find(x => x.id === id), r.find(x => x.id === id), `${at}[${id}]`)).filter(x => x !== undefined);
    }
    ...
    conflicts.push(at); return structuredClone(l);
  };
