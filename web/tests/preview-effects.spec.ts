...
test('native text effect preview is rendered at full image extent', async ({ page, request }) => {
  ...
  const response = await request.get(`/api/document?taskId=${encodeURIComponent(taskId!)}`, {
    headers: { 'X-Canvas-Token': service.token },
  });
  const doc = await response.json();
  const image = doc.images.find((i: any) => i.name === '可编辑文字与原生字效.psd');
  expect(image).toBeTruthy();
  const layer = image.layers.find((l: any) => l.kind === 'text');
  expect(layer.previewUrl).toBeTruthy();
  const mutations: string[] = [];
  page.on('request', (r) => {
    if (r.url().includes('/api/') && ['POST', 'PUT', 'DELETE', 'PATCH'].includes(r.method()))
      mutations.push(r.url());
  });
  await page.goto(`/?taskId=${encodeURIComponent(taskId!)}#token=${service.token}`);
  await page.getByLabel('当前图片').selectOption(image.id);
  ...
  await expect(picture).toHaveCSS('left', '0px');
  await expect(picture).toHaveCSS('top', '0px');
  await expect(picture).toHaveCSS('width', `${image.width}px`);
  await expect(picture).toHaveCSS('height', `${image.height}px`);
  await expect
    .poll(() => picture.evaluate((el: HTMLImageElement) => el.complete && el.naturalWidth > 0))
    .toBe(true);
  const withEffects = await page
    .locator('.image-board.selected')
    .screenshot({ path: 'test-output/native-preview-with-effects.png' });
  // Temporary DOM-only reference rendering checks actual displayed pixels; no document/API writes.
  const before = await picture.evaluate((el: HTMLImageElement) => ({
    src: el.getAttribute('src')!,
    style: el.getAttribute('style')!,
  }));
  await picture.evaluate((el: HTMLImageElement, l: any) => {
    el.src = l.url;
  ...
  await expect
    .poll(() => picture.evaluate((el: HTMLImageElement) => el.complete && el.naturalWidth > 0))
    .toBe(true);
  const withoutEffects = await page
    .locator('.image-board.selected')
    .screenshot({ path: 'test-output/native-preview-raw-reference.png' });
  await picture.evaluate((el: HTMLImageElement, state) => {
  ...
  await expect
    .poll(() => picture.evaluate((el: HTMLImageElement) => el.complete && el.naturalWidth > 0))
    .toBe(true);
  const a = await sharp(withEffects).removeAlpha().raw().toBuffer();
  const b = await sharp(withoutEffects).removeAlpha().raw().toBuffer();
  ...
  fs.writeFileSync(
    'test-output/native-preview-pixel-check.json',
    JSON.stringify(
      {
        taskId,
        imageId: image.id,
        ...
        mutatingRequests: mutations.length,
      },
      null,
      2,
    ),
