...
test('completed native artifact downloads authenticated original bytes', async ({ page }) => {
  ...
  page.on('request', request => { if (request.url().includes('/api/') && ['POST','PUT','DELETE','PATCH'].includes(request.method())) mutations.push(request.url()); });
  const taskId = process.env.LAYER_CANVAS_ARTIFACT_TASK;
  test.skip(!taskId, 'Set LAYER_CANVAS_ARTIFACT_TASK to a task containing a real completed native artifact.');
  const service = JSON.parse(fs.readFileSync('data/service.json', 'utf8'));
  const jobs = JSON.parse(fs.readFileSync('data/jobs.json', 'utf8'));
  const job = jobs.findLast((j: any) => j.taskId === taskId && j.status === 'completed' && j.result?.artifactUrl?.endsWith('.ai'));
  expect(job, 'A real completed Illustrator artifact must exist').toBeTruthy();
  const expectedPath = path.join('data', 'documents', job.documentId, 'assets', path.basename(job.result.artifactUrl));
  ...
  const psdOption = page.getByLabel('当前图片').locator('option').filter({ hasText: '可编辑文字与原生字效.psd' });
