...
export function originalReference(documentId: string, current: CanvasImage) {
  ...
  const archived = json<Record<string, CanvasImage>>(
    path.join(dir(documentId), 'deleted-images.json'),
    {},
  );
  let image = current;
  ...
  return {
    imageId: image.id,
    url: image.source,
    path: assetPath(documentId, image.source),
    width: image.width,
    height: image.height,
    complete: !image.parentId,
    readOnly: true,
