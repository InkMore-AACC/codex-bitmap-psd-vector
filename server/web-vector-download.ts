...
export function reviewAddress(value: string) {
  ...
  if (
    url.origin !== 'https://vectorizer.com' ||
    !/^\/review\/[a-f0-9]{16}\/?$/.test(url.pathname) ||
    url.search ||
    url.hash
  )
...
export async function receiveInContext(
  ...
  try {
    ...
    if (page.url() !== address)
      throw new VerificationRequired('结果页发生跳转；请在当前网页确认验证和结果是否仍有效');
    const names = page.locator('input.name-input');
    try {
      await names.first().waitFor({ state: 'visible', timeout: 15000 });
    } catch {
      throw new VerificationRequired(
        '下载接收器未能访问结果页；请在当前网页确认验证及结果是否仍有效',
      );
    }
    const available = await names.evaluateAll((elements) =>
      elements.map((el) => (el as HTMLInputElement).value || el.getAttribute('placeholder')),
    );
    ...
    if (
      ...
      expected.some((n) => !available.includes(n))
    )
    ...
    for (const [index, input] of inputs.entries()) {
      ...
      try {
        ...
        if (stat.size <= 40 * 1024 * 1024) {
          cleanSvg(await fs.readFile(destination, 'utf8'));
          valid = true;
        }
      } catch {}
      if (!valid) {
        ...
        const row = page
          .locator('.review')
          .filter({ has: page.getByPlaceholder(name, { exact: true }) });
        const [download] = await Promise.all([
          page.waitForEvent('download', { timeout: 45000 }),
          row.getByRole('button', { name: 'DOWNLOAD', exact: true }).click(),
        ]);
        if (download.suggestedFilename() !== name + '.svg') throw new Error('下载文件名称不匹配');
        const downloadedPath = await download.path();
        signal.throwIfAborted();
        if (!downloadedPath || (await fs.stat(downloadedPath)).size > 40 * 1024 * 1024)
          throw new Error('下载文件无效或超过 40 MB');
    ...
    await fs.writeFile(
      path.join(output, 'web-download-manifest.json'),
      JSON.stringify({ reviewUrl: address, files }, null, 2),
    );
    return files;
  } finally {
...
export async function receiveWebVectors(
  reviewUrl: string,
  inputs: DownloadInput[],
  output: string,
  signal: AbortSignal,
  ...
  for (const channel of ['msedge', 'chrome']) {
    try {
      browser = await chromium.launch({ channel, headless: true, timeout: 20000 });
      break;
    } catch {}
  }
