...
export class VectorizerBrowser {
  ...
  async open() {
    for (const channel of ['msedge', 'chrome']) {
      try {
        this.browser = await chromium.launch({ channel, headless: true, timeout: 20000 });
        return;
      } catch {
        /* try the other installed browser */
      }
    }
  ...
  async convert(data: Buffer, signal: AbortSignal, progress: (text: string) => void) {
    ...
    try {
      return await convertInContext(context, data, signal, progress);
    } finally {
...
export async function convertInContext(
  ...
  try {
    ...
    await page
      .locator('#fileInput')
      .setInputFiles({ name: 'layer.png', mimeType: 'image/png', buffer: data });
    // The review link acquires its public result URL after conversion completes.
    await page.waitForFunction(
      () => document.querySelector('#reviewBtn')?.getAttribute('href')?.startsWith('/review/'),
      undefined,
      { timeout: 120000 },
    );
    signal.throwIfAborted();
    ...
    signal.throwIfAborted();
    if (!file || !/\.svg$/i.test(downloaded.suggestedFilename()))
      throw new Error('网页没有返回 SVG 文件');
    ...
    throw new Error(
      '免费网页转换失败：网站超时、页面变化或需要人工验证。原图已保留，请稍后重试或选择其他引擎',
    );
  } finally {
